...
// #define ZONE_ONE      (*((volatile uint32_t *)(0x42000000 + (0x400253FC-0x40000000)*32 + PIN_Z1*4)))
// #define ZONE_TWO      (*((volatile uint32_t *)(0x42000000 + (0x400253FC-0x40000000)*32 + PIN_Z2*4)))
// #define ZONE_THREE    (*((volatile uint32_t *)(0x42000000 + (0x400253FC-0x40000000)*32 + PIN_Z3*4)))
...
void initHw()
{
    ...
    enablePinPullup(PUSH_BUTTON);
      ...
      //FOR PROJECT 2
      GPIO_PORTB_DIR_R |= ZONE_ONE_MASK | ZONE_TWO_MASK | ZONE_THREE_MASK;  // bits 1 and 3 are outputs
      GPIO_PORTB_DR2R_R |= ZONE_ONE_MASK | ZONE_TWO_MASK | ZONE_THREE_MASK; // set drive strength to 2mA (not needed since default configuration -- for clarity)
      GPIO_PORTB_DEN_R |= ZONE_ONE_MASK | ZONE_TWO_MASK | ZONE_THREE_MASK;  // enable LEDs
}
    ...
    if (kbhitUart0())
    {
        ...
        else
        {
            ...
            if (strcmp(token, "mqtt") == 0)
            {
                ...
                {
                    topic = strtok(NULL, " ");
                     data = strtok(NULL, ""); // Grabs the rest of the line
                     if (topic != NULL && data != NULL)
                     {
                         publishMqtt(topic, data);
                         processCommand(topic, data);
                     }
                }
...
int main(void)
{
    ...
    setEtherMacAddress(2, 3, 4, 5, 6, 127);  // my x value
